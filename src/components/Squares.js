...
export default function Squares() {
	...
	const getClassName = square => {
		...
		return activeSquare === square ? 'active' : '';
	};

	const markActive = square => {
		...
		activeSquare === square ? setActiveSquare(null) : setActiveSquare(square);
			...
			<div className="squares">
				{
					...
					squares.map(square => (
						<div
							id={square}
							key={square}
							className={`square ${getClassName(square)}`}
							onClick={() => markActive(square)}
						></div>
					))
				}
			</div>
