...
/* STEP 1 */
const [spinnerState, setSpinnerOn] = useState(true)

  const toggleSpinner = () => {
    setSpinnerOn(!spinnerState)
  /* STEP 4 */
  };
  ...
  return (
    <div className='widget-spinner container'>
      <h2>Spinner</h2>
      {
        spinnerState && <div id='spinner' className='spinner'>--+--</div> /* STEP 2 */
      }
      <button id='toggleSpinner' onClick={toggleSpinner}>
        {(spinnerState) ? 'hide' : 'show'} Spinner {/* STEP 3 */}
      </button>
    </div>
  );
}
