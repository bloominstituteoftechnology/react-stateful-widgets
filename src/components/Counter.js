...
export default function Counter() 
{
  ...
  const increment = () => 
  {
    //When invoked, adds one to count
    setCount (count + 1);
  }
  ...
  const decrement = () => 
  {
    //When invoked, subtracts one from count
    setCount (count - 1);
  }
  ...
  const reset = () => 
  {
    //This function sets the count to 0 on click
    setCount(0);
  }

  //Define styles, assign to style
  const style = 
  {
    fontSize: '1.5em',
    marginBottom: '0.3em',
  ...
  return (
    <div className = 'widget-counter container'>
      ...
      <div id='count' style={style}>
        {/* STEP 3 
          * Replace 0 with {count} and even with result of % logic
          * If count is even: "even*, else: "odd" */}
