...
import React, { useState }from 'react'; /* <<<STEP 0-- importing the state hook. */
...
*/
  ...
  const decrement = () => {
    /* STEP 5 This click handler needs to use 'setCount' to set the 'count' 
 ...
 */
  ...
  const reset = () => {
    /* STEP 6 This click handler needs to use 'setCount' 
    to set the 'count' to be the initial number you chose.*/
    setCount(0)
  };
  // If count is even, then "royalblue", else "crimson".
  const style = {
...
// step 3 below We need to replace some hard-coded info in the JSX with expressions, interpolated inside curly brackets.
  // Start by replacing the character "0" with {count}. The 'count' slice of state is the source of truth here.
  // Then, replace the word "even" with a ternary: {if count is even number, then string "even", else string "odd"}.
  return (
