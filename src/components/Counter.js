...
export default function Counter() {
  ...
  const increment = () => {
    setCount(count + 1);    // setCount is a partially preset function that since it is paired with const count above, when invoked, will return
  //  the value of the contents within it's "arguments/parrameters" and toss them back into the current state value of 'count'
  };
  const decrement = () => {
    setCount(count - 1);
  };
