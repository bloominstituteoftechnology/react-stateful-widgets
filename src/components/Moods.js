...
const initialMood = 'cOnFuSIoN';
const happyMood = 'Happier than hell';
const sadMood = 'adder than a toad';

export default function Moods() {
  /* STEP 1 */
  const [mood, setMood] = useState('cOnFuSIoN');

  const makeHappy = () => {
    /* STEP 4 */
    setMood('Happier than hell');
  };
  const makeSad = () => {
    /* STEP 5 */
    setMood('Sadder than a toad')
  };
  const reset = () => {
    /* STEP 6 */
    setMood('cOnFuSIoN')
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: (mood === 'Happier than hell') ? 'royalblue' : 'crimson', /* STEP 2 */
  };
