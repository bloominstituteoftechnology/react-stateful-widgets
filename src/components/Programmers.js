...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return featuredProgrammers
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: !featuredProgrammers ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {!featuredProgrammers ? setfeaturedProgrammers(dev.name) : setfeaturedProgrammers(null)}}>Feature</button>
            </div>
          )
        }
      </div>
