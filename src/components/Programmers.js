...
export default function Programmers() {
  ...
  const [peoples, setPeoples] = useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
    ...
    return peoples[featured - 1].name
      ...
      <div className='programmers'>
        {
          ...
          peoples.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setFeatures(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
