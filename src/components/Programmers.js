...
export default function Programmers() {
  ...
  const [idOfFeatured, setIdOfFeatured] = useState(null);

  console.log(programmers.id);
  console.log(idOfFeatured, "idoffeatured outside")
  ...
  const getNameOfFeatured = () => {
    ...
    const featDevObj = programmers.find(dev => idOfFeatured === dev.id)
    console.log(featDevObj);
    return featDevObj.name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (idOfFeatured) ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setIdOfFeatured(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
