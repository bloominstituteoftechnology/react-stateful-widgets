...
export default function Programmers() {
  ...
  const [programmerName, featuredName] = useState('');
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: true ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          list.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => featuredName(dev.id)}>Feature</button>
            </div>
          )
        }
      </div>
