...
export default function Programmers() {
  ...
  let [listOfGrammers,setListOfGrammers] = useState(listOfAwesome); 
  const getNameOfFeatured = () => {
    ...
    return listOfGrammers.find(val=>val.id == featured).name
  };
  const isFeaturing = () => listOfGrammers.some(val=>val.id==featured)
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: isFeaturing() ? 'gold':'royalblue', // 🤔 color turns to gold, when celebrating
  };
