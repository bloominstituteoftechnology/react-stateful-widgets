...
export default function Programmers() {
  ...
  var [programmerList, setProgrammerList] = useState(null);
  var [ids, setIds] = useState (null);
  ...
  const getNameOfFeatured = () => {
    ...
    // of the program, without needing to inject the information through argumentss.
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setIds(ids = dev.id); setProgrammerList(programmerList = dev.name)}}>Feature</button>
            </div>
          )
        }
      </div>
