...
const [idz, setId] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    programmers.forEach(programmer => {
     idz == programmer.id ? dev = programmer.name : null
    })
    return dev
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (idz != null) ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
  ...
  return (
    <div className='widget-programmers container'>
      ...
      <div id='featured' style={style}>
        {idz != null ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
