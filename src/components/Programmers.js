...
export default function Programmers() {
  ...
  // The programmers list on the one hand, and the id of the featured programmer on the other.
    var [programmer, setprogrammer] = useState(listOfAwesome)
    var [xid, setxid] = useState(null)
  const getNameOfFeatured = () => {
    ...
    return programmer[xid -1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (xid >= 0) ? "gold" : "royalblue" // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmer.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setxid(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
