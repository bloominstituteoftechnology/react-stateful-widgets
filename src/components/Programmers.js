...
export default function Programmers() {
  ...
  const [developers, setDevelopers] = useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
    ...
    developers.forEach( dev => {
      ...
      if(activeId === dev.id){
       console.log('its working', dev.name)
       name =  dev.name;
      }
    })
    return name;
      ...
      <div className='programmers'>
        {
          ...
          developers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={(e) => { setActiveId(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
