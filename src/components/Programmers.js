...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    // of the program, without needing to inject the information through arguments.
      ...
      // console.log(`id of featured: ${feat}`)
      const featuredDev = prog.filter(x => (x.id === feat));
      // console.log(featuredDev[0].name);
      return featuredDev[0].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: feat === null? 'royalblue': 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          prog.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {feat === dev.id? setFeat(null) : setFeat(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
