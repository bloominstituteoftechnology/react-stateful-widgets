...
export default function Programmers() {
  ...
  const [programmer, setProrammer] = useState(listOfAwesome)
  const [idFeatures, setIdFeatures] = useState(null)
  const getNameOfFeatured = () => {
    ...
    console.log(programmer,idFeatures)
    return programmer[idFeatures - 1].name
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idFeatures ? 'gold':'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmer.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() =>  {setIdFeatures(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
