...
let randomNumber = () => {
  return Math.floor(Math.random() * listOfAwesome.length)
}
...
export default function Programmers() {
  ...
  // const [featuredProgrammer, setFeaturedProgrammer] = useState(listOfAwesome[number].id)
  ...
  const getNameOfFeatured = () => {
    ...
    // return listOfAwesome[Number(featuredProgrammer)].name 
    const list = listOfAwesome.filter((x) => {
      return x.id === featuredProgrammer 
    })
