...
export default function Programmers() {
  ...
  const getNameOfFeatured = (id) => {
    ...
    return programmers.find(current => current.id === activeProgrammer).name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programmers == activeProgrammer ? "gold" : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={(id) => { /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
