...
export default function Programmers() {
  ...
  const [programer,setProgramer] = useState(listOfAwesome);
   const [featuredProgramer, setFeaturedProgrammer] = useState('');
   let nameid='' ;
  const getNameOfFeatured = () => {
   ...
   // const nameProg = programer.map((prog)=>{ if(prog.id === featuredProgramer){
     return programer[featuredProgramer-1].name;
     // nameid=prog.name;
// return  nameid;
//     }
...
//     return nameid;
//     }      )
//     return nameid; //setFeaturedProgrammer(programer.featuredProgramer) ;
 };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredProgramer=='' ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div id='featured' style={style}>
        {
          ...
          featuredProgramer =='' ? 'Pick an awesome programmer' : `🎉 Let's celebrate ${getNameOfFeatured()}  ! 🥳`
