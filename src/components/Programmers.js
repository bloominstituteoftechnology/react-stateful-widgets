...
const [devs, setDevs] = useState(listOfAwesome)
const [ids, setIds] = useState(null)

  const getNameOfFeatured = () => {
   return (devs[ids -1].name)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color:  ids != null ? 'gold' : 'royalblue'  // 🤔 color turns to gold, when celebrating
  };
      ...
      <div id='featured' style={style}>
        {
          ...
          ids != null ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
