...
export default function Programmers() {
  ...
  const [id, setId] = useState(listOfAwesome)
  const [currentProgrammer, setCurrentProgrammer] = useState(0)
  ...
  const getNameOfFeatured = () => {
    ...
    return id[currentProgrammer - 1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (currentProgrammer > 0) ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
