...
export default function Programmers() {
  ...
  const [idFeatProgram, setIdFeatProgram] = useState(null)
  const getNameOfFeatured = () => {
    ...
    return programmers[idFeatProgram-1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idFeatProgram !== null ? 'gold' : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setIdFeatProgram(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
