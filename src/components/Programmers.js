...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return listOfDev[featId -1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featId != null ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfDev.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeatId(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
