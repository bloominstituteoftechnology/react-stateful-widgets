...
export default function Programmers() {
  const [programmers, programmersId] = useState(listOfAwesome)
  const [currentProgrammer, changeProgrammer] = useState()
  // We'll have to use the state hook twice, as we need two slices of state.
  // The programmers list on the one hand, and the id of the featured programmer on the other.

  const getNameOfFeatured = () => {
    return currentProgrammer.name
    // Leave this for last!
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentProgrammer ? 'gold' : 'royalBlue'
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {changeProgrammer(dev)}}>Feature</button>
            </div>
          )
        }
      </div>
