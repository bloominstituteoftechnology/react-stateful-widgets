...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const programmer = programmers.find(elem => elem.id === idy)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idy ? 'gold' : 'royalBlue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setIdy(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
