...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    //*loop over array of devs, check each one, return name that = id that's in state
    // for (let i = 0; i < devList.length; i++) {
    //   if (devList[i].id === featured) {
    //     return devList[i].name;
    //   }
    // }
//*REFACTORED vv

    const foundDev = devList.find(dev => dev.id === featured);
