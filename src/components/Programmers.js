...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    if(feature === null){
      ...
      const findFeat = list.find( x => {
        return feature === x.id
      })

    return findFeat.name 
    } 
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: feature == null ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div id='featured' style={style}>
        {
          ...
          //false
            ...
            feature !== null ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome Programmer!'
        }
      </div>
    </div>
  );
