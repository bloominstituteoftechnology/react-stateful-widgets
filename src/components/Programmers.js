...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const dev = programmerList.find(e => 
      e.id === activeProgrammer
  )
  return dev.name
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activeProgrammer !== null ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmerList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setActiveProgrammer(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
