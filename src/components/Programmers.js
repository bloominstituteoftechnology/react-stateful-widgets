...
export default function Programmers() {
  ...
  const [displayProgramer, setDisplayProgramer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    if(displayProgramer !== null) {
      return programList[displayProgramer - 1].name
    }
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (displayProgramer === null ? 'royalblue' : "gold") // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setDisplayProgramer(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }</div>
      <div style={style} id = "featured">
      {
         ...
         displayProgramer !== null
          ?`🎉 Let's celebrate ${getNameOfFeatured()}! 🥳`
          : "Pick an awesome programmers"
}
    </div>
    </div>
